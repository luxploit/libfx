...
class fxCipherTwofish {
    symmetric_CBC cbc_{};
    bool initialized_ = false;
public:
    fxCipherTwofish() = default;

    fxCipherTwofish(const std::uint8_t* key, std::size_t keylen) {
        this->init(key, keylen);
    }

    fxCipherTwofish(std::vector<std::uint8_t> key) {
    ...
    fxCipherTwofish(const fxCipherTwofish&) = delete;
    fxCipherTwofish& operator=(const fxCipherTwofish&) = delete;
    ...
    bool init(const std::uint8_t* key, const std::size_t keylen) noexcept {
        ...
        TOMCRYPT_ASSERT(errc, "Twofish-CBC start failed!", {})
    ...
    [[nodiscard]] std::vector<std::uint8_t> encrypt(const std::uint8_t* iv, const std::uint8_t* plaintext, const std::size_t pt_len) noexcept {
        ...
        TOMCRYPT_ASSERT(errc, "Twofish-CBC encrypt failed!", {})
        return ciphertext;
    }
    ...
    [[nodiscard]] std::vector<std::uint8_t> decrypt(const std::uint8_t* iv, const std::uint8_t* ciphertext, const std::size_t ct_len) noexcept {
        ...
        TOMCRYPT_ASSERT(errc, "Twofish-CBC decrypt failed!", {})
        return plaintext;
    }
